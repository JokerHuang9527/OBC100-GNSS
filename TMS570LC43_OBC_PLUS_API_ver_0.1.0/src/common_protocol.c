...
/* ------------------------------------------------------------------------------------------------------------------------------
 * Function : CP_PackerInitialManual
 * Description : 
 *      - Initial packer manual, enter your custom pointer to packer, reference blow:
 *          - dataArray = calloc(configArraySize, sizeof(CP_ElementData));
 *          - buffer = calloc(packer->maxBufferSize, sizeof(uint8_t));
 * ----------------------------------------------------------------------------------------------------------------------------*/
...
/* ------------------------------------------------------------------------------------------------------------------------------
 * Function : CP_ClearPackage
 * Description : 
 *      - You need to use this function after you finish process data inside packer
 *      - Ready to receive next package
 * ----------------------------------------------------------------------------------------------------------------------------*/
void CP_ClearPackage(CP_PackagePacker *packer)
{
    CP_PackerDataClear(packer);
    CP_GetNextElement(packer);
}
...
/* ------------------------------------------------------------------------------------------------------------------------------
 * Function : CP_GetElementValue
 * Description : 
 *      - Get element from packer, you need to enter index and it will put data to your pointer
 * ----------------------------------------------------------------------------------------------------------------------------*/
...
/* ------------------------------------------------------------------------------------------------------------------------------
 * Function : CP_SetElementValue
 * Description : 
 *      - Set element to packer, you need to enter index and it will set data from your pointer
 * ----------------------------------------------------------------------------------------------------------------------------*/
void CP_SetElementValue(CP_PackagePacker *packer, uint32_t index, void *data)
{
    index = packer->elementIndex - 1;
    if (packer->configArray[index].flag & CP_FLAG_DATA_REVERSE)
...
/* ------------------------------------------------------------------------------------------------------------------------------
 * Function : CP_ChecksumCalculate
 * Description : 
 *      - Calculate element which have CP_FLAG_CHECKSUM_CALCULATE flag
 * ----------------------------------------------------------------------------------------------------------------------------*/
unsigned long long CP_ChecksumCalculate(CP_PackagePacker *packer)
{
    unsigned long long checksum = 0;
    unsigned long long mask = 0;
    int i,arrayIndex,bufferIndex;
    for (i = 0; i < packer->data->size; i++)
        mask = (mask << 8) | 0xff;

    for (arrayIndex = 0; arrayIndex < packer->elementSize; arrayIndex++)
    {
        if (!(packer->configArray[arrayIndex].flag & CP_FLAG_CHECKSUM_CALCULATE))
            continue;

        for (bufferIndex = 0; bufferIndex < packer->dataArray[arrayIndex].size; bufferIndex++)
...
#endif
    return dataChecksum == CP_ChecksumCalculate(packer);
}

/* ------------------------------------------------------------------------------------------------------------------------------
 * Function : CP_ElementCheck
 * Description : 
 *      - Check special element, like magic code, checksum, crc ...etc
 * ----------------------------------------------------------------------------------------------------------------------------*/
int CP_ElementCheck(CP_PackagePacker *packer)
{
...
/* ------------------------------------------------------------------------------------------------------------------------------
 * Function : CP_GetBindingLength
 * Description : 
 *      - Search config if it defined a CP_ELEMENT_TYPE_LENGTH element before CP_FLAG_LENGTH_BINDING element
 * ----------------------------------------------------------------------------------------------------------------------------*/
...
/* ------------------------------------------------------------------------------------------------------------------------------
 * Function : CP_PackerPutByte
 * Description : 
 *      - Input a byte to packer, callback while detect error or receive a valid package.
 * ----------------------------------------------------------------------------------------------------------------------------*/
...
/* ------------------------------------------------------------------------------------------------------------------------------
 * Function : CP_ElementPutByte
 * Description : 
 *      - Input a byte to element, it will auto switch to next element if received correct data.
 * ----------------------------------------------------------------------------------------------------------------------------*/
int CP_ElementPutByte(CP_PackagePacker *packer, uint8_t byte)
{
    uint32_t index;
    ...
    if (packer->config->type == CP_ELEMENT_TYPE_MAGIC)
    {
        ...
        //index = (packer->config->flag & CP_FLAG_m4bit) ? packer->dataIndex+2 : packer->dataIndex;
...
/* ------------------------------------------------------------------------------------------------------------------------------
 * Function : CP_GeneratePackage
 * Description : 
 *      - Generate package to packer buffer
 *      - You can use this function like printf, put the CP_FLAG_ARG element sequencily
 *  ----------------------------------------------------------------------------------------------------------------------------*/
